...
const generateCreateTableSQL = (dbName, schema, reset) => {
  ...
  const sql = []
  if (reset) {
    sql.push('DROP TABLE IF EXISTS ' + dbName)
  }
  sql.push('CREATE TABLE IF NOT EXISTS ' + dbName + ' (' + fields + ')')
  return sql
}

// insert an array of changes into the database
const insertBulk = async (createSQL, dbName, theSchema, batch) => {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
